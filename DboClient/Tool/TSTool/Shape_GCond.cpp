...
IMPLEMENT_SERIAL( CShape_GCond, CShape_GEntityCont, 1 )


CShape_GCond::CShape_GCond( const CPoint &point )
: CShape_GEntityCont( point, _T("CT_GCond") )
{
	AddAttr( _T("rm"), CString( _T("0") ) );
}

CShape_GCond::~CShape_GCond( void )
{
}

void CShape_GCond::ShowContAttrEditBox( void )
{
	CAttr_CONT_GCond AttrPage;
	AttrPage.AddAttr( _T("cid"), m_strContId );
	AttrPage.AddAttr( _T("rm"), GetAttr( _T("rm") ) );
	...
	if ( IDOK == clDiag.DoModal() && clDiag.GetEditedAttrPage() )
	{
		...
		strValue = AttrPage.GetAttr( _T("rm") );
		if ( strValue.GetLength() != 0 ) SetAttr( _T("rm"), strValue );
...
void CShape_GCond::ShowEntityAttrAddBox( void )
{
...
void CShape_GCond::GetProperty( CNtlTSScrProperty& clProperty )
{
	clProperty.m_defProperty["cid"] = m_strContId;
	clProperty.m_defProperty["rm"] = GetAttr( _T("rm") );
}
