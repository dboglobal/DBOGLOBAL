...
#include "platform_common/Header_C.h"
#include "config.h"
...
#define MUST_CONVERT(enc, s) (!(enc)->isUtf16 || (((tUnsigned32)s) & 1))
...
tSigned32 XML_GetCurrentByteIndex(XML_Parser parser)
{
  if (eventPtr)
    return parseEndByteIndex - (tSigned32)(parseEndPtr - eventPtr);
  return -1;
}
