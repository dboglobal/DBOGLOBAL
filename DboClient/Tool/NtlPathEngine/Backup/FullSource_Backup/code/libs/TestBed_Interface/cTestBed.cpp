...
#include "libs//TestBed_Interface/cObstacleRenderInfo.h"
...
void cTestBed::setMesh( const char* filename )
{
	iMesh* mesh;
	{
		char *buffer;
		unsigned long size;
		buffer = loadBinary(filename, size);
		mesh = nSingletons::pathEngineI().loadMeshFromBuffer("tok", buffer, size, 0);
		freeBuffer(buffer);
	}
	...
	_mesh = reinterpret_cast<cMesh*>(mesh);
	if(_mesh->getTestBedUserData() == 0)
	{
		_mesh->setTestBedUserData(new cTestBedUserData);
		...
		if(!meshUserData()._baseCircuitsRecorder)
		{
			...
			meshUserData()._obstacleRednerInfo = new cObstacleRenderInfo;
			meshUserData()._obstacleRednerInfo->Build( *baseCircuits, gip, _mesh->refTranslation() );
			meshUserData()._obstacleRednerInfo->BuildVertexBuffer();
		}
	}
...
cPosition cTestBed::positionAtMouse( tSigned32 screenx, tSigned32 screeny )
{
	if(!_mesh)
		return cPosition();
	...
	_pickScreenX=screenx;
	_pickScreenY=screeny;
...
cTestBed::drawBurntInObstacles()
{
	...
	if( meshUserData()._obstacleRednerInfo )
	{
		meshUserData()._obstacleRednerInfo->Render();
	}
