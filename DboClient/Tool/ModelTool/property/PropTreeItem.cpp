...
#define PROPTREEITEM_EXPANDBOX			13			// size of the expand box
...
{
	g_ExpandIcon[0].Create( pDC, IDB_EXPANDLEFT_BITMAP );
	g_ExpandIcon[1].Create( pDC, IDB_EXPANDLEFT_BLUE_BITMAP );
	g_ExpandIcon[2].Create( pDC, IDB_EXPANDDOWN_BITMAP );
	g_ExpandIcon[3].Create( pDC, IDB_EXPANDDOWN_BLUE_BITMAP );
}
...
{		
	...
	if( bFill )
	{
		if( bExpand	) imageindex = 	1;
		else imageindex = 	3;
	}
	else
	{
		if( bExpand	) imageindex = 	0;
		else imageindex = 	2;
	}
	
	CDC* pDC = CDC::FromHandle( hdc );

	g_ExpandIcon[imageindex].PasteToDC( pDC, x, y, g_ExpandIcon[imageindex].m_Size.cx,g_ExpandIcon[imageindex].m_Size.cy );
}
...
CPropTreeItem::CPropTreeItem(void) :
	m_pProp(NULL),
	m_sLabel(_T("")),
	m_sInfo(_T("")),
	m_loc(0,0),
	...
	m_pVis(NULL)
{
	m_ControlType = _XPTI_ControlType_Base;
}
