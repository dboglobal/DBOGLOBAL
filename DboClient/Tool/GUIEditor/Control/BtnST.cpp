...
void CButtonST::setBtnImage( UINT uPopBitmapID, UINT uPushBitmapID )
{ 
	CDC *pDC = GetDC();
	
	POP_BTNIMAGE.Create( pDC, uPopBitmapID );

	if( uPushBitmapID )
		PUSH_BTNIMAGE.Create( pDC, uPushBitmapID );
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
  ...
  if ((m_MouseOnButton == TRUE) || (bIsPressed))
	...
	if( this->Selected )
	{
		if( this->PUSH_BTNIMAGE.m_Bitmap != NULL )
			 this->PUSH_BTNIMAGE.PasteToDC( pDC, 1,1 );	
		else if( this->POP_BTNIMAGE.m_Bitmap ) this->POP_BTNIMAGE.PasteToDC( pDC, 1,1 );	
	}
	else
	{
		if( this->POP_BTNIMAGE.m_Bitmap ) this->POP_BTNIMAGE.PasteToDC( pDC, 1,1 );	
	}
...
*/
  ...
  if( this->Selected && !this->PUSH_BTNIMAGE.m_Bitmap )
  {
	  pDC->Draw3dRect(itemRect, ::GetSysColor(COLOR_BTNHILIGHT),::GetSysColor(COLOR_BTNSHADOW) );	
  }
...
void CButtonST::GetTooltipText( CString* spText )
{			
	(*spText) = m_ToolTipString;
}
